...
const BANK_POI_IDS = Object.values(POIS)
    .filter(poi => poi.activities?.some(act => 
        act.type === 'bank' || 
        (act.type === 'npc' && act.actions?.some(a => 'action' in a && a.action === 'open_bank')) ||
        (act.type === 'slayer_master' && act.actions?.some(a => 'action' in a && a.action === 'open_bank'))
    ))
    .map(poi => poi.id);
